...
from ai_models import AIModelFactory, SUPPORTED_GEMMA_MODELS, validate_api_key, get_model_info
...
with st.sidebar:
    ...
    if analysis_mode in ["AI Image Analysis", "Both"]:
        ...
        ai_model_type = st.selectbox(
            "🧠 AI Model",
            ["Gemini", "Gemma"],
            help="Choose between Google's Gemini or Hugging Face's Gemma models"
        )
        
        # Model-specific settings
        if ai_model_type == "Gemma":
            gemma_model = st.selectbox(
                "📦 Multimodal Model",
                list(SUPPORTED_GEMMA_MODELS.keys()),
                help="Select the specific multimodal model to use"
            )
        ...
        api_key = st.text_input(
            "🔑 API Key",
            type="password",
            help=f"Enter your {ai_model_type} API key"
        )
...
if analysis_mode in ["AI Image Analysis", "Both"]:
    ...
    if not validate_api_key(api_key):
        st.error("❌ Please enter a valid API key to use AI analysis features.")
            ...
            if not question.strip():
                ...
                with st.spinner("🧠 Analyzing image with AI..."):
                    try:
                        ...
                        if ai_model_type == "Gemma":
                            model_name = SUPPORTED_GEMMA_MODELS[gemma_model]
                            ai_model = AIModelFactory.create_model("gemma", api_key, model_name=model_name)
                        else:
                        ...
                        if ai_model:
                            ...
                            if result["success"]:
                                ...
                                <div class="ai-analysis-card error">
                                    ...
                                    <div style="margin-top: 1rem; padding: 1rem; background: var(--surface-color); border-radius: 8px; border-left: 4px solid var(--accent-color);">
                                        <h5>💡 Try LLaVA Instead!</h5>
                                        <p style="font-size: 0.9rem; color: var(--text-secondary);">
                                            Switch to <strong>LLaVA models</strong> in the sidebar - they're completely free with no rate limits!
                                        </p>
                                        <ul style="font-size: 0.9rem; color: var(--text-secondary); margin: 0.5rem 0;">
                                            <li>✅ No rate limits or quotas</li>
                                            <li>✅ No billing required</li>
                                            <li>✅ Multiple model sizes available</li>
                                            <li>✅ Same multimodal capabilities</li>
                                        </ul>
                                    </div>
                                </div>
                                """, unsafe_allow_html=True)
...
with st.expander("🔧 Setup Instructions"):
    ...
    4. **Copy Token**: Copy the token and paste it in the sidebar
    ...
    - **Models**: 2B, 9B, 27B parameter versions
